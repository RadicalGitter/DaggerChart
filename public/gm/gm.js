  ...
  $("#folk-grid").innerHTML = S.characters
    .map((c) => {
      ...
      return `<div class="card" style="${dead ? "opacity:0.6;" : ""}">
        ...
        <p style="font-size:0.9rem;">${esc(c.backstory || "")}</p>
        ...
        <details class="gm-only">
          ...
          <div class="inner">
            ...
            <div class="muted" style="font-size:0.82rem;">${esc(c.hidden?.notes || "")}</div>
          </div>
        </details>
        <div style="margin-top:0.6rem;"><button class="quiet" data-edit="${c.id}">Edit</button></div>
      </div>`;
    })
    .join("");
...
function renderFolkEditor(c) {
  const isNew = !c;
  c = c || { name: "", role: "", status: "alive", backstory: "", traits: {}, aptitudes: {}, publicTraits: true, hidden: {} };
  ...
  $("#folk-editor").innerHTML = `<div class="card" style="max-width:560px; margin-top:1rem;">
    ...
    <div class="formrow" style="align-items:flex-start;"><label>Backstory</label><textarea id="ed-backstory" rows="3">${esc(c.backstory || "")}</textarea></div>
